...
from bch_ibm_decode import BCH63_51_Decoder  # 使用你的 class

# -------- helpers --------
def bits_to_int(bits: List[int]) -> int:
    """LSB at index 0 -> integer."""
    v = 0
...
# -------- generator --------
def gen_one_sigma(dec: BCH63_51_Decoder, t_max: int = 2) -> Tuple[List[List[int]], List[int]]:
    n = 63
    t = random.randint(0, t_max)
    err_pos = sorted(random.sample(range(n), t)) if t > 0 else []
    rx = [0]*n
    for p in err_pos:
        rx[p] ^= 1  # 僅錯誤圖樣即可（syndrome 只與錯誤相關）
    S = dec.syndromes(rx)
    sigma_bits = dec.berlekamp_iBM(*S)  # list of [10b] bit-lists, sigma0..sigma2
...
def main():
    ...
    ap.add_argument("--outdir", default="bch_63_chien_search_tp")
    args = ap.parse_args()
    ...
    for _ in range(args.num):
        ...
        # 第二組（僅 i_mode==1 才產、且 testbench 會多收一筆 golden）
        if i_mode == 1:
    ...
    print(f"[BCH63_51] PATTERN_LEN={args.num}, PATTERN_TWO_LEN(請在 TB 設定)={two_out_cnt}")
