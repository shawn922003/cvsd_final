...
class BCH255_239_Decoder:
    ...
    def berlekamp_iBM(self, S1: List[int], S3: List[int]) -> Tuple[int, int, int, int, bool]:
        """
        Returns:
            sigma0, sigma1, sigma2, L_est, degree_gt_t

        Λ̂(z) = σ0 + σ1 z + σ2 z^2  (for T=2)
        L_est 是 Berlekamp 原演算法中估計的 degree L^(2t)，
        degree_gt_t = (L_est > T) 可用來判斷「實際錯誤數 > t → 視為解碼失敗」。
        ...
        # Iterate r = 0..2t-1, after loop we have state at r = 2t
        for r in range(2 * T):
            phi0 = Phi[0]
            psi0 = Psi[0]
