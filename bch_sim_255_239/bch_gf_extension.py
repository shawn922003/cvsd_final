...
# -------------------------
# GF(2^8) 向量(bit-serial)版
# p(x) = 1 + x^2 + x^3 + x^4 + x^8  →  x^8 ≡ 1 + x^2 + x^3 + x^4
# 元素表示：LSB-first list [a0..a7]
# -------------------------
class GF2mVector:
    def __init__(self):
        self.m = M
        # 溢出時 XOR 的約簡 taps：x^8 ≡ 1 + x^2 + x^3 + x^4 → [1,0,1,1,1,0,0,0]
        self.reduction_taps = PRIMITIVE_POLY[:-1]  # 去掉最高次項 x^8
    ...
    #         約簡：把 x^8 拆成 1 + x^2 + x^3 + x^4，並加到結果上
    #     '''
    #     self.check_len(a)
    #     carry = a[7]
    #     b = [0]*8
